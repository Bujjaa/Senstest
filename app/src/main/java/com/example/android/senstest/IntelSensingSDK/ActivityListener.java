...
public class ActivityListener implements ContextTypeListener {
    ...
    public void onReceive(Item state) {
        if (state instanceof com.intel.context.item.ActivityRecognition) {
            ...
            EventBus.getDefault().post(new ActivityEvent(((ActivityRecognition) state).getMostProbableActivity().getActivity() + " " + ((ActivityRecognition) state).getMostProbableActivity().getProbability()));
            SharedPreferences prefs = Application.getContext().getSharedPreferences("Data", Context.MODE_PRIVATE);
            SharedPreferences.Editor editor = prefs.edit();
            editor.putString("Activity",((ActivityRecognition) state).getMostProbableActivity().getActivity() + " " + ((ActivityRecognition) state).getMostProbableActivity().getProbability());
